...
/* adapted to work with hifi-web - @kordero */
var GOL = {
  ...
  colors : {
    ...
    {
      dead : '#FFFFFF',
      trail : ['#9898FF', '#8585FF', '#7272FF', '#5F5FFF', '#4C4CFF', '#3ca8cc', '#2626FF', '#1313FF', '#0000FF', '#1313FF', '#2626FF', '#3ca8cc', '#4C4CFF', '#5F5FFF', '#7272FF', '#8585FF'],
      alive : ['#000000']
    }
  ...
  init : function(config, state) {
    try {
      ...
      this.loadConfig(config); // Load config (autoplay, colors, zoom, ...)
      this.loadState(state);   // Load state
  ...
  loadConfig : function(config) {
    ...
    colors = parseInt(config.colors || this.helpers.getUrlParameter('colors'), 10);
    if (isNaN(colors) || colors < 1 || colors > GOL.colors.schemes.length) {
      colors = 1;
    }

    // Initial grid config
    grid = parseInt(config.grid || this.helpers.getUrlParameter('grid'), 10);
    if (isNaN(grid) || grid < 1 || grid > GOL.grid.schemes.length) {
      grid = 1;
    }

    // Initial zoom config
    zoom = parseInt(config.zoom || this.helpers.getUrlParameter('zoom'), 10);
  ...
  /**
       ...
       */
  loadState : function(initialState) {
    var state, i, j, y, s = initialState || this.helpers.getUrlParameter('s');
